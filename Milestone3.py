...
class CWMachine(StateMachine):
    ...
    def toggleMessage(self):
        if self.activeMessage == self.message1:
            self.pendingMessage = self.message2
        else:
            self.pendingMessage = self.message1


        if(DEBUG):
            print(f"* Toggling active message to: {self.pendingMessage} ")
        ...
        while not self.endTransmission:
            ...
            for word in wordList:
                ...
                if wordsCounter < lenWords:
                    ...
                    self.doWP()
                    wordsCounter += 1
                elif hasattr(self, 'pendingMessage'):
